...
class PayDialog(QDialog, Ui_PayDialog):
    ...
    def pay_clicked(self):
        ...
        if pay_amount == "":
            standartbox("No value to pay given!", parent=self, devenvironment=self.devenvironment)
        else:
            # rounds the number down to two digits (best is to limit it even that way)
            pay_amount = round(float(pay_amount),2)
            user_return = standartbox("Pay amount entered is: {} €".format(pay_amount), boxtype="okcancel", okstring="Enter", parent=self, devenvironment=self.devenvironment)
        ...
        if enter_credit:
            ...
            standartbox("Your payment has been entered!", parent=self, devenvironment=self.devenvironment)
    ...
    def undo_clicked(self):
        ...
        user_return = standartbox("Remove the last entry for this employee?", boxtype="okcancel", okstring="Yes", cancelstring="No", parent=self, devenvironment=self.devenvironment)
        if user_return == 1024:
